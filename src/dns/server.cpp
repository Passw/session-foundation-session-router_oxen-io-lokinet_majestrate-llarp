...
namespace srouter::dns
{
    ...
    namespace libunbound
    {
        ...
        class Resolver final : public Resolver_Base, public std::enable_shared_from_this<Resolver>
        {
            ...
            const net::Platform* net_ptr() const { return srouter::net::Platform::Default_ptr(); }
            ...
            void configure_upstream(const srouter::DnsConfig& conf)
            {
...
#undef CLOSE
            ...
            srouter::DnsConfig m_conf;

          public:
            explicit Resolver(quic::Loop& loop, srouter::DnsConfig conf) : _loop{loop}, m_conf{std::move(conf)}
            {
                up(m_conf);
            }
            ...
            void up(const srouter::DnsConfig& conf)
            {
                if (m_ctx)
    ...
    std::shared_ptr<I_Platform> Server::create_platform() const
    {
        ...
        if constexpr (srouter::platform::has_systemd)
        {
    ...
    std::shared_ptr<PacketSource> Server::make_packet_source_on(const quic::Address& addr, const srouter::DnsConfig&)
    {
