...
namespace srouter::dns
{
    ...
    {
      public:
        using ReplyCallback = std::function<void(Message&& response)>;

        explicit RequestHandler(Router& router);

        // Called when a request arrives to process the request; when the answer is ready, calls
        // `reply()` with it.
        void operator()(std::span<const std::byte> request, const quic::Address& from, ReplyCallback reply);
      ...
      private:
        ...
        bool handle_local(ReplyCallback& reply, Message& msg, std::string qname);
        ...
        bool handle_local_ptr(Message& m, ReplyCallback& reply);

        // Answers the question recursively via our configured upstream DNS servers (if any)
        void forward(Message&& m, ReplyCallback&& reply);
    };
