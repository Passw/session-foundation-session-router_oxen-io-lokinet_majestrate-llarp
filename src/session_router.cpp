        ...
        std::function<void()> on_timeout)
    {
        ...
        if (session->is_established()) {
            ...
            if (on_established || on_timeout) {
                ...
                osession->on_established([ti, on_established, on_timeout] (const srouter::session::OutboundSession& session) {
                    if (session.is_established()) {
                        if (on_established)
                            on_established(ti);
                    } else{
            ...
            log::warning(logcat, "Unexpected: tunnel session returned a non-established, but also non-outbound session!");
        }
